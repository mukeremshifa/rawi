#!/usr/bin/env node
/**
 * Contrast, computed rather than asserted (Rawi design system).
 *
 * The palette in `src/client/styles/globals.css` is written in OKLCH, which no WCAG
 ...
 *
 * Wired into `verify` as `check:contrast --fail`. Gamut clipping means a pair
...
const PAIRS = [
  ...
  // The evidence ramp, in both of its roles (rule 4 in globals.css).
  //
  // FIELD: ink sits on every stop, because one rule styles the whole set — so all four have to clear 4.5 on ink.
  ['evidence-ink', 'evidence-none', 4.5, 'ink on the not-checked field'],
  ['evidence-ink', 'evidence-practicing', 4.5, 'ink on the practicing field'],
  ['evidence-ink', 'evidence-independent', 4.5, 'ink on the independent-once field'],
  ['evidence-ink', 'evidence-retained', 4.5, 'ink on the retained-on-review field'],
  // MARK: each stop sitting on the page — an evidence dot, a chart
  // stroke, an icon. This is the pair the old single ramp failed at 1.22:1.
  ['evidence-none-mark', 'background', 3.0, 'the not-checked mark against the page'],
  ['evidence-practicing-mark', 'background', 3.0, 'the practicing mark against the page'],
  ['evidence-independent-mark', 'background', 3.0, 'the independent-once mark against the page'],
  ['evidence-retained-mark', 'background', 3.0, 'the retained-on-review mark against the page'],
  ['evidence-none-mark', 'card', 3.0, 'the not-checked mark against a card'],
  ['evidence-retained-mark', 'card', 3.0, 'the retained-on-review mark against a card'],
];
...
const RAMPS = {
  field: ['evidence-none', 'evidence-practicing', 'evidence-independent', 'evidence-retained'],
  mark: ['evidence-none-mark', 'evidence-practicing-mark', 'evidence-independent-mark', 'evidence-retained-mark'],
};
const rampRows = [];
...
console.log('\nContrast — computed from src/client/styles/globals.css\n');
console.log(
...
console.log('\nEvidence ramp lightness — must climb, so value carries what hue cannot\n');
